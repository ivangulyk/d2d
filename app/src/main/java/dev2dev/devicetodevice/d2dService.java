...
public class d2dService extends Service implements WifiP2pManager.PeerListListener{//, WifiP2pManager.ConnectionInfoListener {
    ...
    public IBinder onBind(Intent intent) {
        switch (intent.getAction()) {
            ...
            case "CHAT_ACTIVITY":
                return null;//this.ChatActivityBinder;
            default:
                return null;
        }
    }
  ...
  //  private final ChatBinder ChatActivityBinder = new ChatBinder();
    ...
    public class PeerBinder extends Binder {
        ...
        public List<PeerDevice> getDevices() {
            ...
            for (WifiP2pDevice p2pDevice : d2dService.this.Devices) {
                //ClientInfo comparable = ClientInfo.GetComparable(p2pDevice.deviceAddress);
                PeerDevice device = new PeerDevice(p2pDevice);//, d2dService.this.Clients.contains(comparable));
                devices.add(device);
            }
            return devices;
        }
